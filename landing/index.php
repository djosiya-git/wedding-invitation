...
$heroShots = [];
foreach (['animation', 'minimalist', 'vintage'] as $heroKey) {
    $firstTemplate = $groups[$heroKey]['templates'] ? reset($groups[$heroKey]['templates']) : null;
    $heroShots[] = [
        'label' => $groups[$heroKey]['label'] ?? ucfirst($heroKey),
        'price' => $groups[$heroKey]['price_label'] ?? '',
        'image' => $firstTemplate['thumbnail_url'] ?? '',
    ];
}
?>
<!doctype html>
...
<body>
  ...
  <section class="hero">
    <div class="hero-stage" aria-hidden="true">
      <?php foreach($heroShots as $index=>$shot): ?>
        <div class="phone-shot <?=e(['shot-a','shot-b','shot-c'][$index])?>"><div class="phone-ui"><div class="phone-art"><img src="<?=$adminPrefix?>/<?=e($shot['image'] ?: app_logo_url())?>" alt=""></div><div class="phone-copy"><b><?=e($shot['label'])?></b><span><?=e($shot['price'])?></span></div></div></div>
      <?php endforeach ?>
    </div>
            ...
            <div class="templates">
              <?php $i=0; foreach($group['templates'] as $templateKey=>$template): $i++; ?>
                <article class="template <?=$i>5?'is-hidden':''?>" data-name="<?=e(strtolower($template['name']))?>">
                  <a class="template-thumb <?=$template['thumbnail_url']?'has-image':''?>" target="_blank" rel="noopener" href="<?=$adminPrefix?>/template_preview.php?template=<?=urlencode($templateKey)?>">
                    <?php if($template['thumbnail_url']): ?>
                      <img class="template-cover" loading="lazy" src="<?=$adminPrefix?>/<?=e($template['thumbnail_url'])?>" alt="<?=e($template['name'])?>">
                    <?php else: ?>
